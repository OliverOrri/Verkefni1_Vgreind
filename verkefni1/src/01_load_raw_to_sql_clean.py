...
def main():
    ...
    # Your 00_fetch creates columns like: Mánuður, value, source, fetched_at
    # We'll support either:
    #  - month_code + value_text
    #  - or Mánuður + value
    # Normalize columns:
    if "month_code" not in wage_raw.columns and "Mánuður" in wage_raw.columns:
        wage_raw = wage_raw.rename(columns={"Mánuður": "month_code"})
    if "value_text" not in wage_raw.columns and "value" in wage_raw.columns:
        wage_raw = wage_raw.rename(columns={"value": "value_text"})

    if "month_code" not in cpi_raw.columns and "Mánuður" in cpi_raw.columns:
        cpi_raw = cpi_raw.rename(columns={"Mánuður": "month_code"})
